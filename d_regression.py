...
class SPMM_regressor(nn.Module):
    def __init__(self, tokenizer=None, config=None):
        ...
        for i in range(bert_config.fusion_layer, bert_config.num_hidden_layers):  self.text_encoder.bert.encoder.layer[i] = nn.Identity()#fudion_layer设为空
        self.text_encoder.cls = nn.Identity()#设为Identity，不进行任何操作
...
def train(model, data_loader, optimizer, tokenizer, epoch, warmup_steps, device, scheduler):
    ...
    for i, (text, value) in enumerate(tqdm_data_loader):
        ...
        tqdm_data_loader.set_description(f'loss={loss.item():.4f}, lr={optimizer.param_groups[0]["lr"]:.6f}')#设置进度条的描述
...
def main(args, config):
    ...
    if name == 'bace':
        ...
        dataset_val = SMILESDataset_BACER('data/4_MoleculeNet/BACER_test.csv')
        dataset_test = SMILESDataset_BACER('data/4_MoleculeNet/BACER_valid.csv')
    elif name == 'lipo':
    ...
    with open("result_1.txt", "a") as f:
        ...
        for epoch in range(0, max_epoch):
            ...
            lr_scheduler.step(epoch + warmup_steps + 1)#调整学习率
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', default='/home/syy/model_pretrain_CL_v2/a_ours_model/model_save/model_save_927/checkpoint_step=1090000.ckpt')
    parser.add_argument('--vocab_filename', default='./vocab_bpe_366.txt')
